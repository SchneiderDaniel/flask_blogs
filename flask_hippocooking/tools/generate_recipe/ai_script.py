...
def process_and_save_image(source_path, recipe_id):
    """Verarbeitet das Bild und speichert es unter ../../static/recipes_images/<id>.jpg"""
    try:
...
# --- CORE AGENT ---
def run_agent(recipe_text, recipe_id, source_image_path):
    api_key = load_api_key()
    if not api_key:
        messagebox.showerror("Error", "Missing API Key in .geminikey.txt")
        return
    
    # 1. Bild verarbeiten, falls eines ausgewählt wurde
    ...
    try:
        ...
        success_msg = f"Recipe ID {recipe_id} generated and saved."
        if source_image_path:
            success_msg += "\nImage was also processed and saved." if image_processed else "\nWarning: Image processing failed."
...
def on_preview():
    ...
    tk.Label(preview_win, text=f"Target ID: {recipe_id}", font=("Arial", 10, "bold")).pack(pady=5)
...
def on_submit():
    ...
    if not validate_id(recipe_id):
        ...
        return
    if not text:
        messagebox.showwarning("Warning", "Please fill in recipe text.")
        return
        
    run_agent(text, recipe_id, source_img)

# --- UI SETUP ---
root = tk.Tk()
root.title("Recipe AI Generator & Image Processor")
...
tk.Label(root, text="Select Image (Optional)", font=("Arial", 10, "bold")).pack(pady=(15, 0))
...
tk.Button(btn_frame, text="🚀 Generate & Process", command=on_submit, bg="#2e7d32", fg="white", font=("Arial", 10, "bold"), width=25, height=2).pack(side=tk.LEFT, padx=10)
